			...
			<div class="admin-content-body">
				<div class="am-cf am-padding am-padding-bottom-0">
					<div class="am-fl am-cf">
						<strong class="am-text-primary am-text-lg">HOME</strong> / <small>全部管理</small>
					</div>
				</div>
						...
						<form id="admins" class="am-form">
							<table
								...
								<thead>
									<tr>
										...
										<th>姓名</th>
										<th>密码</th>
										<th class="am-hide-sm-only">身份证号码</th>
								...
								<tbody>
									<c:forEach items="${adminList}" var="admin">
										<tr>
											<td><input name="ids" type="checkbox" value="${admin.adminId}" /></td>
											<td>${admin.adminId}</td>
											<td>${admin.adminName}</td>
											<th>${admin.adminPwd}</th>
											<th class="am-hide-sm-only">${admin.adminIcard}</th>
											<th class="am-hide-sm-only">${admin.adminPhone}</th>
											<td>
												<div class="am-btn-toolbar">
													<div class="am-btn-group am-btn-group-xs">
														<button type="button" onclick="location.href='${pageContext.request.contextPath}/admin/editAdmin?adminId=${admin.adminId}'"
															class="am-btn am-btn-default am-btn-xs am-text-secondary" title="编辑">
															<span class="am-icon-pencil-square-o"></span>
														</button>
														<button type="button" onclick="return delAdmin('${admin.adminId}')"
								...
								function delAll() {
									...
									onConfirm: function(e) {
										$.ajax({
											type:'POST',
											async:false,
											url:'${pageContext.request.contextPath}/admin/delAllAdmins',
											data:$('#admins').serialize(),
											dataType:'json',
											success:function(data){
												alert(data.result);
												window.location.reload();
							...
							<div class="am-cf">
								...
								<div class="am-fr">
									<ul class="am-pagination">
										<div data-am-page="{pages:${pageCount},curr:${nowPage},jump:'${pageContext.request.contextPath}/admin/allAdmin?nowPage=%page%&size=${size}'}"></div>
									</ul>
								</div>
							</div>
							<!-- 每页显示大小下拉 -->
							<script type="text/javascript">
								...
								$("#tableSize").change(function () {  
						            ...
						            window.location.href='${pageContext.request.contextPath}/admin/allAdmin?size='+size;
						        });
							</script>
							<hr />
							<p>注：.....</p>
